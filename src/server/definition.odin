...
get_all_package_file_locations :: proc(
	...
	for match in matches {
		encoded_path := common.make_encoded_path(match, context.temp_allocator)
...
get_definition_location :: proc(req_ctx: ^RequestContext) -> ([]common.Location, bool) {
	...
	if position_context.import_stmt != nil {
		if get_all_package_file_locations(req_ctx.doc_ctx, position_context.import_stmt, &locations) {
		   ...
		   position_context.identifier != nil {
			...
			if resolved, ok := resolve_location_identifier(&ast_context, ident^); ok {
				...
				if resolved.filepath == "" {
					location.uri = common.make_encoded_path(req_ctx.doc_ctx.filepath, context.temp_allocator)
				} else {
