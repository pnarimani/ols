...
get_definition_location :: proc(doc_ctx: DocumentContext, position: common.Position, config: ^common.Config, symbols: ^SymbolCollection) -> ([]common.Location, bool) {
	...
	position_context, ok := get_document_position_context(doc_ctx, position, .Definition)
	...
	if position_context.import_stmt != nil {
		if get_all_package_file_locations(doc_ctx, position_context.import_stmt, &locations) {
